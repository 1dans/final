...
void main_process(vector<vector<int>> matrix, int row, int col) {
    cout << "в розробці"; //здесь должно быть: трек пустой ячейки(отдельная функция), перемещение (поместить в while(!win) win отдельная функция для определения, выиграл пользователь или нет) перемещение должно быть реализовано так: пользователь называет какую клетку хочет переместить (например а2) на место пустой, программа должна отслеживать, возможно ли перемещение) если цифра на своем месте то пусть подсвечивается зеленым
}
